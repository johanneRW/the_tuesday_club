...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('myapp', '0002_adminalbumview_albumview'),
    ]
...
albumimage.image_name AS album_image
FROM
(
   (
      (
         (
            (
               (
                  myapp_album album
                  LEFT JOIN myapp_artist artist ON
                  (
                     (album.artist_id = artist.artist_id)
                  )
               )
               LEFT JOIN myapp_albumunitformat albumunitformat ON
               (
                  (album.album_id = albumunitformat.album_id)
               )
            )
            LEFT JOIN myapp_albumformat albumformat ON
            (
               (albumunitformat.album_format_id = albumformat.id)
            )
         )
         LEFT JOIN myapp_label label ON ((album.label_id = label.label_id))
      )
      LEFT JOIN myapp_albumprice albumprice ON
      (
         (album.album_id = albumprice.album_id)
      )
   )
   LEFT JOIN myapp_albumimage albumimage ON
   (
      (
         ...
         (
            ...
            migrations.RunSQL(
                """
                CREATE OR REPLACE VIEW admin_album_view AS
                SELECT
album.album_id,
album.album_name,
artist.artist_name,
...
(
   (
      ...
      LEFT JOIN myapp_albumeancode eancode ON ((album.album_id = eancode.album_id))
   )
   LEFT JOIN myapp_albumupc upccode ON ((album.album_id = upccode.album_id))
)
WHERE
(
   albumprice.price_start_date =
